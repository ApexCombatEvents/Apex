...
CREATE POLICY IF NOT EXISTS "Public read access avatars"
...
CREATE POLICY IF NOT EXISTS "Public read access banners"
ON storage.objects FOR SELECT
...
CREATE POLICY IF NOT EXISTS "Public read access post-images"
ON storage.objects FOR SELECT
...
CREATE POLICY IF NOT EXISTS "Public read access event-banners"
ON storage.objects FOR SELECT
USING (bucket_id = 'event-banners');

-- Allow authenticated users to upload event banners
CREATE POLICY IF NOT EXISTS "Authenticated users can upload event-banners"
ON storage.objects FOR INSERT
WITH CHECK (
...
CREATE POLICY IF NOT EXISTS "Users can update own event-banners"
ON storage.objects FOR UPDATE
USING (
  bucket_id = 'event-banners' 
  AND auth.uid()::text = (storage.foldername(name))[1]
...
CREATE POLICY IF NOT EXISTS "Users can delete own event-banners"
ON storage.objects FOR DELETE
USING (
  bucket_id = 'event-banners' 
  AND auth.uid()::text = (storage.foldername(name))[1]
