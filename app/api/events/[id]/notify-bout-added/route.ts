// app/api/events/[id]/notify-bout-added/route.ts
...
export async function POST(
  ...
  try {
    ...
    let boutId: string;
    let boutNumber: number | null;
    let cardType: string | null;
    
    try {
      const body = await req.json();
      boutId = body.boutId;
      boutNumber = body.boutNumber || null;
    ...
    const { data: bout } = await supabaseAdmin
      .from("event_bouts")
      .select("id, red_name, blue_name, card_type")
      .eq("id", boutId)
      .single();

    const redName = bout?.red_name || "TBC";
    const blueName = bout?.blue_name || "TBC";
    const finalCardType = cardType || bout?.card_type || null;
    ...
    console.error("Notify bout added error:", error);
    return NextResponse.json(
